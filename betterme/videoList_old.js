    ...
    else if(status == UPDATE_DATA_STATUS.SUCCEED)
    {
      ...
      show_view = <View>
        ...
        <View style={{flex:1,flexDirection:"row",padding:12,backgroundColor:"white"}}>
          ...
          <View style={{flex:1,justifyContent:"center",alignItems:"flex-end"}}>
            <TouchableOpacity style={{borderWidth:1,padding:4,borderRadius:4}} onPress={()=>this.props.navigation.navigate("Packages")}>
              <Text style={{fontSize:16}}>{'选专辑'}</Text>
            </TouchableOpacity>
          </View>
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    packages:(page,callback)=>{
      dispatch(packages({utype_id:utype_id,page:page},callback))
    },
